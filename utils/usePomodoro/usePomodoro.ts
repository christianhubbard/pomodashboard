...
export const usePomodoro = (config: PomodoroConfig = defaultConfig) => {
  ...
  const tick = useCallback(() => {
    if (!currentTimerLength || !endTimestamp)
  ...
  const changeState = useCallback(
    (newState: Partial<PomodoroState>) => {
      currentTimerLength = newState.timer;
      endTimestamp = new Date().valueOf() + newState.timer! * 1000;
      dispatch(changeStateAction(newState));
    },
    [dispatch]
  );
